...
SELECT mf2.OModulo,mf2.OMov,mf2.OMovID,C.Origen,C.OrigenID,d.Aplica,d.AplicaID,d.Importe
 ...
 BEGIN  
  ...
  BEGIN  
   ...
   SELECT TOP(1) @Seleccionar = Seleccionar, @LiberarEjecutivoS = LiberaEjec, @LiberaGerenteS = LiberaGerente, @ProcesadoM = Procesado, @Eliminar = NoDPP, @ImporteTotalX = Total, @DescuentoX = ImporteDesc, @Modificar = Modificar  FROM NaviExploraProntoPa
go WHERE ID = @IDInsertar AND  Empresa = @EmpresaInsertar AND  Mov = @MovInsertar AND  MovID = @MovIDInsertar /*AND  FechaPago = @FechaPagoInsertar*/ AND  Aplica = @AplicaInsertar AND  AplicaID = @AplicaIDInsertar AND IDAplica = @IDAplicaInsertar  
   DELETE FROM NaviExploraProntoPago 
    WHERE ID = @IDInsertar 
 	  AND Empresa = @EmpresaInsertar 
	  AND Mov = @MovInsertar 
 ...
 WHERE n2.ID IS NULL AND n2.Empresa IS NULL AND n2.Mov IS NULL AND n2.MovID IS NULL AND n2.FechaPago IS NULL AND n2.Aplica  IS NULL AND n2.AplicaID IS NULL AND n2.IDAplica IS NULL AND ISNULL(n.NoDPP,0) = 1
	...
	DELETE n FROM NaviExploraProntoPago n JOIN NaviExploraProntoPagoEliminadas n2 ON n.ID = n2.ID AND n.Empresa = n2.Empresa AND n.Mov = n2.Mov AND n.MovID = n2.MovID /*AND n.FechaPago = n2.FechaPago*/ AND n.Aplica = n2.Aplica AND n.AplicaID = n2.AplicaID AN
D n.IDAplica = n2.IDAplica
