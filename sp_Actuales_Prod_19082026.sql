...
GO
--exec spNaviExploraProntoPago 'NVK','JQUEZADA','01/06/2026',N'19/08/2026',N'0'
...
BEGIN        
 ...
 BEGIN  
  ...
  SELECT DISTINCT                         c.ID,                       
                                      c.Empresa,     
                                        ...
                                        Cte.Nombre,           
                                          ...
                                          c.Importe,            
                                          c.Impuestos,
										  ISNULL(NCA.Importe,0)  + ISNULL(NCNR.Importe,0),
										  ISNULL(NCA.Impuestos,0),
                                          c.Retencion,         
                                          ISNULL(P.IMPORTE, ISNULL(antc.anticipo, ant.Importe)) as Anticipo,    
                                          ISNULL(C.Saldo,0),
										  ISNULL(DATEDIFF(DD, C.FechaEmision, P.FechaOriginal),ISNULL(DATEDIFF(DD, C.FechaEmision, ant.FechaOriginal),DATEDIFF(DD, C.FechaEmision, antc.FechaOriginal))) as Dias,
  ...
  JOIN MovTipo M ON C.Mov = M.Mov AND M.Modulo = 'CXC' AND M.CLAVE='CXC.F'
...
/***************************************** Cobros y aplicaciones de anticipos   ******************************************/
  LEFT JOIN   (
                  SELECT  CD.Aplica, CD.AplicaID, c. ID, c.mov, c.movid, ABS(cd.importe) AS ANTICIPO, c.FechaEmision, C.FechaOriginal
                  FROM CXC C         
                  JOIN MovTipo M ON C.Mov=M.Mov AND M.Modulo ='CXC' AND M.Clave IN ('CXC.C', 'CXC.ANC')      
                  JOIN CXCD CD ON C.ID = CD.ID        
  ...
  LEFT JOIN   (
				...
				FROM (
					...
					WHERE c.Empresa = @Empresa
					  ...
					  AND c.ID = mf.DID
					  AND mf2.DMov = c.Mov
					  AND mf2.DMovID = c.MovID
				) z
				WHERE z.rn = 1
              )   ant on c.Mov=ant.Aplica AND c.MovID=ant.AplicaID 
...
/***************************************** notas de crédito ******************************************/
  LEFT JOIN  (		
				SELECT Aplica, AplicaID, ID, Importe,Impuestos,Referencia, FechaEmision,FechaOriginal
				FROM 
				(
				SELECT CD.Aplica, CD.AplicaID, C.ID, C.Impuestos,Referencia, c.FechaEmision, c.FechaOriginal
				, SUM(cd.Importe) OVER (PARTITION BY cd.Aplica, cd.AplicaID) AS Importe,
				ROW_NUMBER() OVER (PARTITION BY cd.Aplica, cd.AplicaID ORDER BY c.ID DESC) AS rn
                  FROM CXC  C      
...
LEFT JOIN (
    ...
    FROM (
        ...
        FROM CXC C
        JOIN MovTipo M  ON C.Mov = M.Mov  AND M.Modulo='CXC' AND M.Clave IN ('CXC.C')
        JOIN CXCD CD    ON C.ID = CD.ID
        JOIN MovTipo M2 ON CD.Aplica = M2.Mov AND M2.Modulo='CXC' AND M2.Clave='CXC.F'
        WHERE C.Estatus IN ('PENDIENTE','CONCLUIDO') AND C.Empresa = @Empresa
    ) z
...
SELECT mf2.OModulo,mf2.OMov,mf2.OMovID,C.Origen,C.OrigenID,d.Aplica,d.AplicaID,d.Importe
    FROM Cxc		c
    LEFT JOIN		MovTipo		mt ON mt.Mov=c.Mov AND mt.Modulo='CXC'
    LEFT JOIN		MovFlujo	mf ON c.Mov = mf.dmov AND c.MovID = mf.DMovID
	LEFT JOIN		MovFlujo	mf2 ON mf.OID = mf2.DID
    JOIN			CxcD		d  ON d.ID  = c.ID
   WHERE  C.Empresa = @Empresa
 ...
 END  
    ...
    UPDATE N SET    Descuento=ISNULL(C.Descuento,0),    
					...
					CASE WHEN ISNULL( N.Parcial,0)=0  
                                        THEN ( (Importe+ISNULL(IVA,0)) - (ISNULL(ImporteNC,0)+ISNULL(IVANC,0))  )*(ISNULL(C.Descuento,0)/100)
 ...
 BEGIN  
  ...
  BEGIN  
   ...
   FechaPago,  
   Descuento  ,  
   IIF(ISNULL(@Modificar,0) = 0, ImporteDesc, @DescuentoX)  ,
   @LiberarEjecutivoS  ,  
   Ejecutivo  ,  
   @LiberaGerenteS  ,  
   Gerente   ,  
